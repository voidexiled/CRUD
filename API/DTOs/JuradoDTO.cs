...
namespace API.DTOs
{
    public class JuradoDTO : IJurados
    {
        ...
        {
            var db = dbConnection();
            var sql = @"CALL create_jurado(@curp_jur, @nombre, @apellido1, @apellido2, @contrasena)";
            var result = await db.ExecuteAsync(sql, new { jurado.curp_jur, jurado.nombre, jurado.apellido1, jurado.apellido2, jurado.contrasena });
            return result > 0;
        }
        ...
        {
            var db = dbConnection();
            var sql = @"CALL update_jurado(@curp_jur, @nombre, @apellido1, @apellido2)";
            var result = await db.ExecuteAsync(sql, new { jurado.curp_jur, jurado.nombre, jurado.apellido1, jurado.apellido2 });
            return result > 0;
        }
    }
}